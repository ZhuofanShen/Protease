...
def calculate_tolerable_fitness(row_values, fitness_type):
    ...
    if fitness_type == 'tolerable':
        ...
        fitness = pow(1.2, -(ddG_fold \
                + 2 * average_ddG_bind - ddG_bind_inhibitor - ddG_bind_inhibitor_zscore \
                + 2 * average_ddG_sub - ddG_sub_inhibitor))
    return mutation, fitness

def convert_xls_to_csv(workbook_name, fitness_type):
    workbook_read = xlrd.open_workbook(workbook_name)
    sheet_read = workbook_read.sheet_by_index(0)
    positions = np.empty(0)
    with open(workbook_name[:-4] + '.' + fitness_type + '.pssm', 'w') as pf:
...
def logo_plot(workbook_name, positions, fitness_type):
    df = pd.read_csv(workbook_name[:-4] + '.' + fitness_type + '.pssm', delim_whitespace=True, index_col=0)
    ...
    # logo.fig.show()
    logo.fig.savefig(workbook_name[:-4] + '.' + fitness_type + '.png', bbox_inches='tight', dpi=1200)

if __name__ == '__main__':
    args = parse_arguments()
    positions = convert_xls_to_csv(args.xls, args.fitness_type)
    logo_plot(args.xls, positions, args.fitness_type)
